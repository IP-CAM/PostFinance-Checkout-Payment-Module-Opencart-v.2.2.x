...
class ControllerPaymentPostFinanceCheckout extends AbstractController {
	...
	public function index(){
		...
		$this->response->setOutput($this->loadView("payment/postfinancecheckout", array_merge($storeConfigs, $pageVariables)));
	}
	...
	private function getSettingPageVariables($shops){
		...
		// Form action url
		$data['action'] = $this->createUrl("payment/postfinancecheckout",
				array(
					\PostFinanceCheckoutVersionHelper::TOKEN => $this->session->data[\PostFinanceCheckoutVersionHelper::TOKEN] 
				));
	...
	private function getSettingsPageBreadcrumbs(){
		return array(
			'breadcrumbs' => array(
				...
				array(
					"href" => $this->createUrl("payment/postfinancecheckout",
							array(
								\PostFinanceCheckoutVersionHelper::TOKEN => $this->session->data[\PostFinanceCheckoutVersionHelper::TOKEN] 
							)),
	...
	private function validateStore($store){
		if (!$this->user->hasPermission("modify", "payment/postfinancecheckout")) {
	...
	protected function getRequiredPermission(){
		return 'payment/postfinancecheckout';
	}
}